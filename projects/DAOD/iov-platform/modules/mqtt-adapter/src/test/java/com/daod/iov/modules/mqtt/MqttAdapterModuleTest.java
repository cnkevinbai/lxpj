 ...
 * MQTT 适配器模块单元测试
 */
@DisplayName("MQTT 适配器模块测试")
class MqttAdapterModuleTest {
    
    private MqttAdapterModule module;
    
    @BeforeEach
    void setUp() {
        module = new MqttAdapterModule();
    }
    ...
    void testMetadata() {
        ...
        assertEquals("mqtt-adapter", metadata.getName());
    ...
    void testMetrics() throws ModuleException {
        ...
        // 检查在线客户端指标
        assertTrue(metrics.stream().anyMatch(m -> m.getName().contains("mqtt")));
    }
}